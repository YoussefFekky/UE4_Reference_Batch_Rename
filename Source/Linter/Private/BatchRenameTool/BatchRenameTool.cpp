...
void FDlgBatchRenameTool::ApplyBatchRename(TArray<FAssetRenameData>& AssetsAndNames, const FAssetData& Asset, FAssetRegistryModule& AssetRegistryModule, TArray<FAssetData>& AllAssets)
{
	...
	TArray<FName> DependencyObjectPaths;
	AssetRegistryModule.GetDependencies(Asset.PackageName,DependencyObjectPaths,EAssetRegistryDependencyType::Hard);
	for(FName ObjectPath: DependencyObjectPaths)
	{
		if(ObjectPath.ToString().Contains("/Game/"))
		{
			TArray<FAssetData> ReferencedAsset;
			AssetRegistryModule.Get().GetAssetsByPackageName(ObjectPath, ReferencedAsset);
			AllAssets.AddUnique(ReferencedAsset.Top());
		}
	}
}

void FDlgBatchRenameTool::ApplySuffix(TArray<FAssetRenameData>& AssetsAndNames, const FAssetData& Asset, FAssetRegistryModule& AssetRegistryModule, TArray<FAssetData>& AllAssets)
{
	// Early out on assets that can not be renamed
	if (!(!Asset.IsRedirector() && Asset.AssetClass != NAME_Class && !(Asset.PackageFlags & PKG_FilterEditorOnly)))
	{
	...
	TArray<FName> DependencyObjectPaths;
	AssetRegistryModule.GetDependencies(Asset.PackageName,DependencyObjectPaths,EAssetRegistryDependencyType::All);
	for(FName ObjectPath: DependencyObjectPaths)
	{
		if(ObjectPath.ToString().Contains("/Game/"))
		{
			TArray<FAssetData> ReferencedAssets;
			AssetRegistryModule.Get().GetAssetsByPackageName(ObjectPath, ReferencedAssets);

			for (FAssetData EAssetData : ReferencedAssets)
			{
				AllAssets.AddUnique(EAssetData);
			}
		}
	}
}
...
FDlgBatchRenameTool::EResult FDlgBatchRenameTool::ShowModal()
{
	...
	if (UserResponse == EResult::Confirm)
	{
		...
		for (auto AssetIt = SelectedAssets.CreateConstIterator(); AssetIt; ++AssetIt)
		{
			...
			ApplyBatchRename(AssetsAndNames, Asset, AssetRegistryModule, SelectedAssets);
		}
...
void FDlgBatchRenameTool::IgnoreAssets(bool bIgnore)
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
	TArray<FAssetRenameData> AssetsAndNames;
	...
	Suffix = "_ig";
	bRemoveSuffix = !bIgnore;
	
	for (auto AssetIt = SelectedAssets.CreateConstIterator(); AssetIt; ++AssetIt)
	{
		const FAssetData& Asset = *AssetIt;
		ApplySuffix(AssetsAndNames, Asset, AssetRegistryModule, SelectedAssets);
	}
	
	if (!AssetToolsModule.Get().RenameAssets(AssetsAndNames))
	{
		FNotificationInfo NotificationInfo(LOCTEXT("IgnoreFailed", "Ignore operation did not fully complete successfully. Maybe fix up redirectors? Check Output Log for details!"));
		NotificationInfo.ExpireDuration = 6.0f;
		NotificationInfo.Hyperlink = FSimpleDelegate::CreateStatic([]() { FMessageLog("LoadErrors").Open(EMessageSeverity::Info, true); });
		NotificationInfo.HyperlinkText = LOCTEXT("LoadObjectHyperlink", "Show Message Log");
		FSlateNotificationManager::Get().AddNotification(NotificationInfo);
...
void FDlgBatchRenameTool::IgnoreAssets(bool bIgnore, const TArray<FString>& PackagePaths)
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
	FAssetRegistryModule& AssetRegistryModule = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
	Suffix = "_ig";
	...
	for(FString PackagePath: PackagePaths)
	{
		...
		AssetRegistryModule.Get().GetAssetsByPath(FName(PackagePath), FolderAssets, true);
		for (auto AssetIt = FolderAssets.CreateConstIterator(); AssetIt; ++AssetIt)
		{
			const FAssetData& Asset = *AssetIt;
			ApplySuffix(AssetsAndNames, Asset, AssetRegistryModule, FolderAssets);
		}
